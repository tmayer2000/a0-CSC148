...
class Election:
    ...
    def read_results(self, instream: IO[str]) -> None:
        ...
        in the A0 handout.
        """
        line_list = instream.readlines
        for line in line_list:
            split_line = line.split(',')
            self.update_results(split_line[1], split_line[13], split_line[17])
    ...
    def results_for(self, riding: str, party: str) -> Optional[int]:
        ...
        1
        """
        num_parties = len(self._results[riding]) # number of parties in a specific riding
        count = 0                                # where are we in the riding dict
        for parties in self._results[riding]:    # checks all parties in a riding for votes
            if self._results[riding][parties] == 0:
                count += 1
